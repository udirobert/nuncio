...
CRITICAL — SOURCE WEIGHTING: Not all sources are equal. A LinkedIn bio, GitHub README, personal website, or Crunchbase profile is a STABLE identity signal — weight these heavily. Individual tweets or social posts are EPHEMERAL and may not represent the person's core interests or identity. Do NOT build a profile narrative around 1-2 tweets. If the only data you have is a few tweet snippets, mark confidence as "low" on relevance_signals and suggestedAngles, and keep personalization_hooks conservative. A single tweet about a topic does not mean the person "cares about" or "is interested in" that topic — it means they posted about it once. Reserve "interests" for things mentioned across multiple sources or in stable profile text (bio, about page, README). If you see a tweet that contradicts the stable profile, note the contradiction rather than picking one over the other.`;
