...
- Ensure correct `gh` auth profile before push; deploy via SSH to the production server using `scripts/deploy.sh` (see `docs/DEPLOY.md` for env overrides)
...
- **LiveLink pilot guardrails** — credit reservations for training and sessions (including anonymous shares), provider-neutral error surfacing, and workspace/sender allowlist are in. Remaining: idle timeout, server-side duration-aware cleanup, and recorded-video fallback on provider failure.
